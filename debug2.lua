...
local Background = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 300, 0, 150),Size = UDim2.new(0, 650, 0, 420)})
local LeftPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(53, 52, 55),BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 19),Size = UDim2.new(0, 165, 0, 401)})
...
function bringBackOnResize()
    ...
    if (currentX < 0) or (currentX > (viewportSize.X - (sideClosed and 165 or Background.AbsoluteSize.X))) then
        if currentX < 0 then
            currentX = 0
        else
            currentX = viewportSize.X - (sideClosed and 165 or Background.AbsoluteSize.X)
        end
    end
        ...
        if not connections["drag"] then
            connections["drag"] = RunService.RenderStepped:Connect(function()
                ...
                if newPos ~= lastPos then
                    ...
                    local viewportSize = workspace.CurrentCamera.ViewportSize
                    if (currentX < 0 and currentX < currentPos.X) or (currentX > (viewportSize.X - (sideClosed and 165 or TopBar.AbsoluteSize.X)) and currentX > currentPos.X) then
                        if currentX < 0 then
                            currentX = 0
                        else
                            currentX = viewportSize.X - (sideClosed and 165 or TopBar.AbsoluteSize.X)
                        end
                    end
...
function toggleMinimize(override)
    ...
    if closed then
        ...
        TweenService:Create(LeftPanel, TweenInfo.new(0.5), {Size = UDim2.new(0, 165, 0, 0)}):Play()
        wait(0.5)
    else
        TweenService:Create(LeftPanel, TweenInfo.new(0.5), {Size = UDim2.new(0, 165, 0, 401)}):Play()
        wait(0.5)
