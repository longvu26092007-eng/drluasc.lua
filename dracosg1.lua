...
-- ==========================================
-- [ 1.1 ] HỆ THỐNG DI CHUYỂN TWEEN
-- (Sử dụng BodyVelocity an toàn, tránh AntiCheat)
-- ==========================================
...
local function TweenTo(targetCFrame, speed)
    ...
    local tweenObj = TweenService:Create(hrp, TweenInfo.new(timeToTween, Enum.EasingStyle.Linear), {CFrame = targetCFrame})
    ...
    conn = RunService.Heartbeat:Connect(function()
        if _G.CancelTween or not hrp or not hrp.Parent then
            tweenObj:Cancel()
            reached = true
        elseif (hrp.Position - targetCFrame.Position).Magnitude <= 15 then
            tweenObj:Cancel()
            reached = true
        end
    end)
...
-- ==========================================
-- HÀM ATTACK + BRING MOB TỐI ƯU CỰC MƯỢT
-- ==========================================
local function SafeKillMob(targetModel, validNamesList)
    xpcall(function()
        ...
        local dist = (hrp.Position - lockedPos).Magnitude
        local attackPos = CFrame.new(lockedPos + Vector3.new(0, 30, 0), lockedPos)
...
do
    ...
    if hasHeart and hasStorm then
                ...
                while _farmingScale do
                    pcall(function()
                        ...
                        if target then
                            ...
                            TweenTo(CFrame.new(SCALE_POS.Position + Vector3.new(0, 30, 0), SCALE_POS.Position), 300)
                        end
                    end)
                    task.wait(0.2)
                end
        ...
        -- Đã Fix: Nhận trả Quest bằng Server Data, Khóa cứng CFrame khi farm Trúc
        -- ==========================================
        do
            ...
            if emberCount >= EMBER_MIN then
                ...
                -- Hàm kiểm tra quest TỪ SERVER (Không dùng Notification ảo ma nữa)
                local function checkDragonQuest()
                    local hasQuest, mobName, questType, needsClaim = false, nil, nil, false
                    pcall(function()
                        ...
                        -- Nếu Text chứa "Head back to the Dojo", tức là đã xong -> Cần Claim
                        ...
                        -- Nếu không có Quest, thử Request Quest
                        if not questData or not questData.Text or (not hasQuest and not needsClaim) then
                        ...
                        if questData and questData.Text then
                            ...
                            elseif string.find(txt, "Destroy") then
                                hasQuest = true
                                questType  = 2
                            elseif string.find(txt, "Head back to the Dojo") then
                                needsClaim = true
                                hasQuest = true
                            end
                        end
                    end)
                    return hasQuest, mobName, questType, needsClaim
                end
                    ...
                    while _farmingEmber do
                        pcall(function()
                            ...
                            if ember and ember:FindFirstChild("Part") then
                                ...
                                StopTween() -- Hủy lệnh bay đánh quái ngay lập tức
                                local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
                                if hrp then
                                    hrp.Anchored = false -- Đề phòng đang bị khóa bởi farm cây trúc
                ...
                while _farmingEmber do
                    pcall(function()
                        if isCollectingEmber then return end -- Ngưng mọi việc nếu đang nhặt Ember
                        ...
                        local hasQuest, mobName, questType, needsClaim = checkDragonQuest()
                        ...
                        elseif hasQuest then
                            ...
                            if questType == 1 and mobName then
                                ...
                                if target then
                                    ...
                                    TweenTo(CFrame.new(HYDRA_POS.Position + Vector3.new(0, 30, 0), HYDRA_POS.Position), 300)
                                end
                            ...
                            elseif questType == 2 then
                                ...
                                if tree then
                                    local hrp = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
                                    if hrp then
                                        local targetPos = tree.CFrame * CFrame.new(0, 5, 20)
                                        
                                        if (tree.Position - hrp.Position).Magnitude <= 40 then
                                            -- Dừng mọi di chuyển Tween
                                            StopTween()
                                            
                                            -- Khóa cứng nhân vật tại chỗ (Tránh lướt skill)
                                            pcall(function()
                                                hrp.Anchored = true
                                                hrp.CFrame = CFrame.new(hrp.Position, tree.Position)
                                                UseAllSkills()
                                            end)
                                            -- Mở khóa ngay sau khi xả skill xong
                        ...
                        -- Xử lý khi Lỗi mất Quest, ép bay về Dojo nhận lại
                        else
                            StopTween()
                            TweenTo(DOJO_POS)
                        end
