...
-- ==========================================
-- BUTTON TÍCH HỢP: Collect → Go → Drop → Claim
-- ==========================================
...
local function lockDojoButtons(isLocked)
    ...
    btnTichHop.AutoButtonColor = v
end


-- ==========================================
-- TAB TRADE: Chair buttons + phím tắt 1-6
-- ==========================================
local CHAIR_KEY_MAP = {
    [Enum.KeyCode.One]   = 1,
    [Enum.KeyCode.Two]   = 2,
...
local function goToChair(index)
    ...
    toposition(seat.CFrame, function()
        status.Text = "Chair: tới " .. seat.Name .. "!"
    end)
end
...
-- ==========================================
-- TÍCH HỢP: Collect → 3s → Go → 3s → Drop → 2s → Claim
-- ==========================================
btnTichHop.MouseButton1Click:Connect(function()
    ...
    status.Text = "⚡ [1/4] Collect xong! Đợi 3s..."
    task.wait(3)
    ...
    status.Text = "⚡ [2/4] Đã tới! Đợi 3s..."
    task.wait(3)

    -- BƯỚC 3: DROP FRUITS
    status.Text = "⚡ [3/4] Drop Fruits..."
    pcall(function() DropFruits() end)
    status.Text = "⚡ [3/4] Drop xong! Đợi 2s..."
    task.wait(2)
...
-- ==========================================
-- PHÍM TẮT: Ctrl = ẩn/hiện UI, 1-6 = Chair
-- ==========================================
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    -- Phím 1-6: bay đến Chair tương ứng
    local chairIndex = CHAIR_KEY_MAP[input.KeyCode]
    if chairIndex then
        goToChair(chairIndex)
    end
end)
