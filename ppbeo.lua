...
-- ==========================================
-- UI TIẾN TRÌNH (ScreenGui tự tạo, kéo được)
-- ==========================================
-- Xóa UI cũ nếu có
pcall(function()
...
TitleLabel.Text = "🥋 PURPLE BELT FARM"
...
MinBtn.BackgroundColor3 = Color3.fromRGB(100, 30, 180)
MinBtn.Text = "−"
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    ContentFrame.Visible = not _minimized
    MainFrame.Size = _minimized and UDim2.new(0, 320, 0, 36) or UDim2.new(0, 320, 0, 400)
    MinBtn.Text = _minimized and "+" or "−"
end)

-- Helper: tạo label hiển thị
local _labels = {}
local _labelY = 0
...
local function MakeLabel(name, defaultText)
    ...
    lbl.Position = UDim2.new(0, 0, 0, _labelY)
...
local function MakeSeparator()
    ...
    sep.Position = UDim2.new(0, 0, 0, _labelY)
    sep.BackgroundColor3 = Color3.fromRGB(80, 50, 130)
    sep.BorderSizePixel = 0
    sep.Parent = ContentFrame
    _labelY = _labelY + 6
end

local function UpdateLabel(name, text)
    if _labels[name] then _labels[name].Text = text end
end

local function SetLabelColor(name, color)
    if _labels[name] then _labels[name].TextColor3 = color end
end
...
local function MakeButton(text, callback)
    ...
    btn.Position = UDim2.new(0, 0, 0, _labelY)
...
-- Tạo UI labels
MakeLabel("header", "═══ TRẠNG THÁI ═══")
SetLabelColor("header", Color3.fromRGB(138, 43, 226))
MakeSeparator()

MakeLabel("status",    "⏳ Trạng thái: Đang khởi tạo...")
MakeLabel("sea",       "🌊 Sea: ...")
MakeLabel("belt",      "🥋 Belt: ...")
MakeLabel("quest",     "📜 Quest: ...")
MakeLabel("elite",     "👹 Elite Boss: ...")
MakeLabel("progress",  "📊 Elite Killed: ...")
MakeLabel("server",    "🖥️ Server: " .. string.sub(JobId, 1, 20) .. "...")
MakeLabel("weapon",    "⚔️ Weapon: " .. getgenv().PurpleBelt.WeaponType)
MakeLabel("team",      "🏴 Team: " .. getgenv().PurpleBelt.Team)

MakeSeparator()
MakeLabel("header2", "═══ LOG ═══")
SetLabelColor("header2", Color3.fromRGB(138, 43, 226))
MakeSeparator()
...
local function AddLog(msg)
    table.remove(_logLines, 1)
    table.insert(_logLines, "• " .. msg)
    UpdateLabel("log1", _logLines[1])
    UpdateLabel("log2", _logLines[2])
    UpdateLabel("log3", _logLines[3])
...
MakeSeparator()
_labelY = _labelY + 2
local startY = _labelY

local stopBtn = MakeButton("⏹ DỪNG SCRIPT", function()
    getgenv().PurpleBelt.Running = false
    UpdateLabel("status", "⛔ Đã dừng script!")
    SetLabelColor("status", Color3.fromRGB(255, 80, 80))
    AddLog("Script đã dừng bởi người dùng")
end)

local hopBtn = MakeButton("🔄 HOP NGAY", function()
    AddLog("Hop thủ công...")
end)
hopBtn.Position = UDim2.new(0.52, 0, 0, startY)
getgenv()._manualHop = false
hopBtn.MouseButton1Click:Connect(function() getgenv()._manualHop = true end)

-- ==========================================
-- CHỜ GAME LOAD
-- ==========================================
UpdateLabel("status", "⏳ Đang chờ game load...")
...
task.spawn(function()
    xpcall(function()
        if not LocalPlayer.Team then
            ...
            task.wait(2)
            AddLog("Đã join team: " .. getgenv().PurpleBelt.Team)
        end
    end, function(err) warn("[PurpleBelt] Team error:", err) end)
end)
...
UpdateLabel("status", "✅ Game đã load!")
...
task.defer(function()
    pcall(function()
        if ReplicatedStorage:FindFirstChild("Effect") and ReplicatedStorage.Effect:FindFirstChild("Container") and ReplicatedStorage.Effect.Container:FindFirstChild("Death") then
            local dm = require(ReplicatedStorage.Effect.Container.Death)
            local cs = require(ReplicatedStorage.Util.CameraShaker)
            if cs then cs:Stop() end
            if hookfunction then hookfunction(dm, function(...) return ... end) end
        end
    end)
end)
...
local function RemoveBodyClip()
    pcall(function() if HumanoidRootPart and HumanoidRootPart:FindFirstChild("BodyClip") then HumanoidRootPart.BodyClip:Destroy() end end)
end
...
local function IsEliteName(name)
    ...
    for _, n in next, ELITE_NAMES do if name == n or name:find(n) then return true end end
    return false
end

local function CheckTool(v)
    for _, x in next, {LocalPlayer.Backpack, Character} do
        if x then for _, v2 in next, x:GetChildren() do
...
local function CheckInventory(...)
    ...
    for _, v in pairs(inv) do for _, n in next, {...} do if v.Name == n then return true end end end
    return false
end
    ...
    for _, container in next, {workspace.Enemies, ReplicatedStorage} do
        for _, m in next, container:GetChildren() do
            if m:IsA("Model") and m.Name ~= "Blank Buddy" then
                local h, r = m:FindFirstChild("Humanoid"), m:FindFirstChild("HumanoidRootPart")
                if h and r and h.Health > 0 then
                    for _, n in next, args do if m.Name == n or m.Name:lower():find(n:lower()) then return m end end
                end
            end
        end
    end return false
...
local function EquipWeapon(wt)
    ...
    if t and t.ToolTip == wt then return end
    for _, x in next, LocalPlayer.Backpack:GetChildren() do
        if x:IsA("Tool") and x.ToolTip == wt then Humanoid:EquipTool(x) return end
    end
end

local function EliteBossExists()
    for _, name in next, ELITE_NAMES do
        if ReplicatedStorage:FindFirstChild(name) or workspace.Enemies:FindFirstChild(name) then return true end
    end return false
end

-- Tìm tên Elite Boss cụ thể đang có ở server
local function FindEliteBossName()
    for _, name in next, ELITE_NAMES do
        if workspace.Enemies:FindFirstChild(name) then return name, "Enemies" end
        if ReplicatedStorage:FindFirstChild(name) then return name, "Replicated" end
    end return nil, nil
end

-- ==========================================
-- FAST ATTACK (KaitunBoss)
-- ==========================================
...
local function Tween(targetCF, target)
    ...
    if not Character or not Humanoid or Humanoid.Health <= 0 then
        ...
        _conn, _tw, _pp, _isTw = nil, nil, nil, false return
    end
    if targetCF == false then
        if _tw then pcall(function() _tw:Cancel() end) _tw = nil end
        if _conn then _conn:Disconnect() _conn = nil end
        if _pp then _pp:Destroy() _pp = nil end
        _isTw = false return
    end
    if _isTw or not targetCF then return end
    _isTw = true
    local root = Character:FindFirstChild("HumanoidRootPart")
    if not root then _isTw = false return end
    target = target or root
    local dist = (targetCF.Position - target.Position).Magnitude
    local oY = (target ~= root) and CFrame.new(0,30,0) or CFrame.new(0,5,0)
    _pp = Instance.new("Part")
    _pp.Name = "TweenGhost"; _pp.Transparency = 1; _pp.Anchored = true; _pp.CanCollide = false
    _pp.CFrame = target.CFrame; _pp.Size = Vector3.new(50,50,50); _pp.Parent = workspace
    ...
    _conn = RunService.Heartbeat:Connect(function()
        if target and _pp then target.CFrame = _pp.CFrame * oY end
    end)
...
local function KillEliteBoss(bossName)
    xpcall(function()
        for _, v in next, workspace.Enemies:GetChildren() do
            ...
            if vh and vh.Health > 0 and vhrp and v.Name == bossName then
                ...
                if dx*dx+dy*dy+dz*dz <= (getgenv().PurpleBelt.AttackRange^2) then
                    ...
                    if getgenv().PurpleBelt.AutoBuso and not Character:FindFirstChild("HasBuso") then
                        pcall(function() COMMF_:InvokeServer("Buso") end) end
                    if getgenv().PurpleBelt.AutoKen and tick()-lastKenCall >= 10 then
                        lastKenCall = tick()
                        pcall(function() ReplicatedStorage.Remotes.CommE:FireServer("Ken", true) end) end
        ...
        for _, v in next, ReplicatedStorage:GetChildren() do
            ...
            if v:IsA("Model") and vhrp and v.Name == bossName then Tween(vhrp.CFrame) return true end
        end
        return false
    end, function(e) warn("[PurpleBelt] Kill ERROR:", e) return false end)
end
...
local function GetServers()
    ...
    for i = 1, 100 do
        local ok, data = pcall(function() return ReplicatedStorage:WaitForChild("__ServerBrowser"):InvokeServer(i) end)
        if ok and data then local h = false; for _ in data do h = true break end
...
local function HopServer(reason)
    AddLog("Hop: " .. tostring(reason))
    UpdateLabel("status", "🔄 Đang hop server...")
    local Servers = GetServers()
    if not Servers then AddLog("Không lấy được server list") return false end
    local arr = {}
    for jid, v in Servers do arr[#arr+1] = {JobId=jid, Players=v.Count, Region=v.Region} end
    for _ = 1, math.min(#arr, 20) do
        local sd = arr[math.random(1,#arr)]
        if sd and sd.Players < getgenv().PurpleBelt.HopMaxPlayers then
            AddLog("→ Server: " .. string.sub(sd.JobId,1,16) .. " (" .. sd.Players .. "p)")
            pcall(function() ReplicatedStorage:WaitForChild("__ServerBrowser"):InvokeServer("teleport", sd.JobId) end)
            task.wait(10) return true
        end
    end
    AddLog("Không tìm server phù hợp") return false
end

-- ==========================================
-- DOJO BELT DETECTION (Banana file 2)
-- ==========================================
...
-- Elite Hunter
local function AcceptEliteQuest() pcall(function() COMMF_:InvokeServer("EliteHunter") end) end
local function GetEliteProgress()
    local ok, p = pcall(function() return COMMF_:InvokeServer("EliteHunter", "Progress") end)
...
local function GetQuestInfo()
    local ok, r = pcall(function()
        ...
        if q.Visible then
            ...
            return {visible=true, text=t, isElite=IsEliteName(t)}
        end
...
local function WriteCompletedFile()
    pcall(function()
        if writefile then
            writefile(LocalPlayer.Name..".txt", "Completed-ppbelt")
            AddLog("GHI FILE: " .. LocalPlayer.Name .. ".txt")
        end
    end)
end
...
local function AlreadyCompleted()
    local ok, c = pcall(function()
        if readfile and isfile and isfile(LocalPlayer.Name..".txt") then return readfile(LocalPlayer.Name..".txt") end return nil
    end)
...
if AlreadyCompleted() then
    UpdateLabel("status", "✅ Đã hoàn thành từ trước!")
    SetLabelColor("status", Color3.fromRGB(80, 255, 80))
    AddLog("File Completed-ppbelt đã tồn tại. Dừng.")
    return
end
...
task.spawn(function()
    ...
    while getgenv().PurpleBelt.Running do
        xpcall(function()
            -- Manual hop
            if getgenv()._manualHop then
                getgenv()._manualHop = false
                HopServer("Hop thủ công")
                task.wait(getgenv().PurpleBelt.HopWaitTime)
                return
            end
            ...
            UpdateLabel("sea", "🌊 Sea: " .. (isSea3 and "3 ✅" or "Không phải 3 ❌"))
            if not isSea3 then
                UpdateLabel("status", "🌊 Đang teleport đến Sea 3...")
                AddLog("Teleport → Sea 3")
                pcall(function() COMMF_:InvokeServer("TravelZou") end)
                task.wait(5) return
            end

            -- Teleport Hydra + bay đến Dojo
            TeleportToHydra()
            task.wait(1)

            if HumanoidRootPart and (DOJO_POS.Position - HumanoidRootPart.Position).Magnitude > 50 then
                UpdateLabel("status", "✈️ Bay đến Dojo Trainer...")
                Tween(DOJO_POS)
                local t0 = tick()
                repeat task.wait(0.5) until not HumanoidRootPart
                    or (DOJO_POS.Position - HumanoidRootPart.Position).Magnitude <= 50
                    or tick()-t0 > 30
                Tween(false)
                task.wait(1)
            end
            ...
            UpdateLabel("belt", "🥋 Belt: " .. (beltName or "Không xác định"))
            UpdateLabel("progress", "📊 Elite Killed: " .. tostring(GetEliteProgress()))

            -- Quest xong?
            if not progress and not beltName then
                UpdateLabel("status", "🔍 Thử claim quest...")
                ClaimDojoQuest()
                task.wait(1)
                local bn2, pr2 = GetCurrentBeltName()
                if not pr2 and not bn2 then
                    if HasPurpleBelt() then
                        UpdateLabel("status", "🎉 HOÀN THÀNH PURPLE BELT!")
                        SetLabelColor("status", Color3.fromRGB(80, 255, 80))
                        AddLog("*** PURPLE BELT ĐÃ XONG! ***")
                        WriteCompletedFile()
                        getgenv().PurpleBelt.Running = false
            ...
            if beltName and beltName ~= "Purple" then
                UpdateLabel("status", "⚠️ Belt: " .. beltName .. " (chưa phải Purple)")
                AddLog("Belt hiện tại: " .. beltName)
                task.wait(5) return
            end

            -- === PURPLE BELT ===
            if beltName == "Purple" then
                UpdateLabel("status", "🟣 Purple Belt → Farm Elite!")
                AcceptEliteQuest()
                task.wait(1)

                local qi = GetQuestInfo()
                UpdateLabel("quest", "📜 Quest: " .. (qi.visible and qi.text or "Không có"))

                if qi.visible and qi.isElite then
                    local eName, eLoc = FindEliteBossName()
                    UpdateLabel("elite", "👹 Elite Boss: " .. (eName or "Không có") .. " [" .. (eLoc or "?") .. "]")

                    if eName then
                        -- ĐÁNH BOSS
                        UpdateLabel("status", "⚔️ Đang đánh: " .. eName)
                        SetLabelColor("status", Color3.fromRGB(255, 200, 50))
                        ...
                        AddLog(eName .. " đã chết!")
                        UpdateLabel("elite", "👹 " .. eName .. " ĐÃ CHẾT ✅")
                        task.wait(2)

                    else
                        -- Không có Elite → HOP
                        UpdateLabel("status", "🔄 Không có Elite Boss → Hop...")
                        UpdateLabel("elite", "👹 Elite Boss: KHÔNG CÓ ❌")
                        Tween(false); _noclipActive = false; RemoveBodyClip()
                ...
                elseif qi.visible and not qi.isElite then
                    UpdateLabel("status", "🔄 Quest khác (không Elite) → Hop...")
                    AddLog("Quest không Elite → Hop")
                    HopServer("Quest không phải Elite")
                    task.wait(getgenv().PurpleBelt.HopWaitTime)
                    return

                else
                    UpdateLabel("status", "📜 Nhận quest Elite Hunter...")
                    AcceptEliteQuest()
                    task.wait(2)
                end

                -- Check hoàn thành sau khi đánh
    ...
    UpdateLabel("status", "⛔ Script đã dừng")
    _noclipActive = false; RemoveBodyClip(); Tween(false)
end)

-- ==========================================
-- UI AUTO UPDATE (server info mỗi 8s)
-- ==========================================
task.spawn(function()
    while task.wait(8) do
        if not getgenv().PurpleBelt.Running then break end
        xpcall(function()
            UpdateLabel("server",   "🖥️ Server: " .. string.sub(JobId, 1, 24) .. "...")
            UpdateLabel("weapon",   "⚔️ Weapon: " .. getgenv().PurpleBelt.WeaponType)
            UpdateLabel("progress", "📊 Elite Killed: " .. tostring(GetEliteProgress()))
        end, function() end)
    end
end)

AddLog("Script đã load thành công!")
print("[PurpleBelt] Loaded! Để dừng: getgenv().PurpleBelt.Running = false")
