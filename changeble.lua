...
local function InvokeWithTimeout(remote, timeout, ...)
    ...
    if not done then
        warn("[BlueBelt] InvokeServer timeout sau " .. timeout .. "s")
    end
...
local function CheckBlueBelt()
    ...
    if type(inv) == "table" then
        for _, item in pairs(inv) do
            if type(item) == "table" and item.Name == "Dojo Belt (Blue)" then
        ...
        warn("[BlueBelt] Timeout getInventory, thử lại sau 15s")
    end
